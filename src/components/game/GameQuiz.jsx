  ...
  return (
    <div style={{textAlign:'center'}}>
      <div className="card" style={{padding:20}}>
        <div className="title" style={{fontSize:28, marginBottom:12}}>🎁 Выбор ответа</div>
        <div className="subtitle question-text" style={{marginBottom:16, fontSize: 'clamp(20px, 5vw, 24px)'}}>{question}</div>
        <div className="quiz-grid">
          {options?.map((opt, i) => (
            <button key={opt.id || i} onClick={() => setSelected(i)} className={`quiz-btn ${selected===i?'active':''}`} style={{height:60, fontSize:20}}>{opt.text || String(opt)}</button>
          ))}
        </div>
        <div style={{marginTop:12, display:'flex', gap:10, justifyContent:'center'}}>
          <button onClick={submit} className="btn btn-primary" style={{padding:'16px 20px', fontSize:'20px'}}>Выбрать</button>
